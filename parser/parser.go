...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerPrefix(token.FLOAT, p.parseFloatingLiteral)
...
func (p *Parser) parseExpressionStatement() *ast.ExpressionStatement {
	...
	// treat optional period after expression statement
...
func (p *Parser) parseFloatingLiteral() ast.Expression {
	...
	if err != nil {
		msg := fmt.Sprintf("could not parse %q as floating", p.curToken.Type)
		p.errors = append(p.errors, msg)
		return nil
	}
	return &ast.FloatingLiteral{Token: p.curToken, Value: value}
}
...
func (p *Parser) parseExpressionList(end token.TokenType) []ast.Expression {
	...
	if !p.expectPeek(end) {
		fmt.Println("unexpected ch:", "got=", p.curToken, "expected=", end)
		return nil
	}
